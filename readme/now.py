...
def create_colour_circle(size: int) -> Image.Image:
    ...
    for y in range(im.height):
        for x in range(im.width):
            ...
            radius = math.sqrt(pow(x - (im.width / 2), 2) + pow(y - (im.height / 2), 2))
            radius /= size
            radius = min(radius, 0.5) * 2

            # Get angle at this point
            angle = math.atan2((im.width / 2) - x, y - (im.height / 2)) / (2 * math.pi)
        ...
        for hour, timezones in timezones.items():
            ...
            with Image.new('RGBA', outer_ring.size, (0, 0, 0, 0)) as im:
                ...
                for offset, timezone in enumerate(timezones):
                    ...
                    r, g, b = (int(x * 255) for x in (r, g, b))
                    draw.text((offset_x, offset_y), text=timezone, fill=(r, g, b, 255), font=font, anchor='ms')
                    ...
                    outer_ring.paste(im, (round((outer_ring.width - im.width) / 2),
